...
export const CHART_TOOLBAR_HEADER_PAD = 'overflow-visible px-1.5 py-0 @lg:px-2 @lg:py-0.5';

export const CHART_TOOLBAR_ROW_CLASS =
  'flex h-5 min-h-5 w-full items-center justify-between gap-1 overflow-visible sm:gap-1.5 @lg:h-7 @lg:min-h-7 @lg:gap-1.5';

/** Export card header: grows with stacked captions (e.g. Sun Path Color + Radius). */
export const CHART_TOOLBAR_EXPORT_ROW_CLASS =
  'flex w-full min-h-5 items-center justify-between gap-1 sm:gap-1.5 @lg:min-h-7';
...
export const CHART_TOOLBAR_CONTROLS_CLASS =
  'flex h-5 min-h-5 min-w-0 flex-1 items-center gap-1 overflow-visible sm:gap-1.5 @lg:h-7 @lg:min-h-7';

export const CHART_TOOLBAR_ICON_SLOT_CLASS =
  'flex h-4 w-4 shrink-0 items-center justify-center @lg:h-[18px] @lg:w-[18px]';

export const CHART_TOOLBAR_ICON_GLYPH_CLASS = 'h-3 w-3 @lg:h-3.5 @lg:w-3.5';
...
export function chartToolbarTypePillClass(_theme: 'light' | 'dark'): string {
  return `inline-flex h-5 min-w-0 max-w-full items-center overflow-hidden rounded-full transition-[background-color,border-color,box-shadow,padding] duration-200 ease-out @lg:h-7 ${CHART_TYPE_CHIP} focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-400/80 focus-visible:ring-offset-1 dark:focus-visible:ring-orange-500/70 dark:focus-visible:ring-offset-gray-800`;
}
...
export function chartToolbarTypeBadgeClass(_theme: 'light' | 'dark'): string {
  return `inline-flex h-5 shrink-0 items-center justify-center rounded-full @lg:h-7 ${CHART_TYPE_CHIP} ${CHART_TOOLBAR_ICON_SLOT_CLASS}`;
}

export const CHART_TOOLBAR_TITLE_TEXT_CLASS =
  'text-[10px] font-medium leading-none @lg:text-xs';
...
export function chartToolbarTitleClass(theme: 'light' | 'dark'): string {
  return `flex h-5 min-w-0 flex-1 items-center truncate @lg:h-7 ${CHART_TOOLBAR_TITLE_TEXT_CLASS} ${chartToolbarTitleInk(theme)}`;
}
...
export function chartToolbarMenuTriggerClass(theme: 'light' | 'dark'): string {
  return `inline-flex h-5 shrink-0 items-center gap-0.5 rounded-md px-0.5 @lg:h-7 ${CHART_TOOLBAR_TITLE_TEXT_CLASS} ${chartToolbarTitleInk(
    theme
